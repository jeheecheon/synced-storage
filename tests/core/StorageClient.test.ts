...
describe("StorageClient", () => {
  describe("getOrCreateStore", () => {
    ...
    it("different strategies for same key yield different stores", () => {
      const client = new StorageClient();
      const local = client.getOrCreateStore("key", "default", {
        strategy: "localStorage",
      });
      const session = client.getOrCreateStore("key", "default", {
    ...
    it("same key with same strategy returns same instance", () => {
      const client = new StorageClient();
      const s1 = client.getOrCreateStore("key", "d", {
        strategy: "sessionStorage",
      });
      const s2 = client.getOrCreateStore("key", "d", {
        strategy: "sessionStorage",
      });
      expect(s1).toBe(s2);
    });
